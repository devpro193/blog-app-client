...
export async function getServerSideProps({ params, req }) {
    ...
    let data = await fetch(`${process.env.SERVER_URL}/article/draft/${params.draft}`, {
        method: "get",
        headers: new Headers({
            'Authorization': `Bearer ${req.cookies?.parallelVortex}`
        })
    }).then(res => res.json())
...
export default function Editdraft({ id, data, cookies }) {
    ...
    useEffect(() => {
        ...
        document.getElementById("edit").innerHTML = data?.body?.join("") || "Write Something Here..."
    ...
    return (
        <>
            <Head>
                <title>Continue writing article...</title>
            </Head>
            <main className="flex justify-center min-h-[90svh] px-4 lg:px-16">
                <div className="flex  max-lg:w-full w-full max-w-3xl flex-col items-center gap-4 py-5">
                    ...
                    }} contentEditable placeholder="Title" className={`${jose.className} px-3 py-1 outline-none text-3xl w-full border-l-4 resize-none overflow-hidden`} defaultValue={title} rows={2} maxLength={80}></textarea>
